...
static int qmiclientId[QMUX_TYPE_MAX];
...
static int GobiNetGetClientID(const char *qcqmi, UCHAR QMIType) {
    ...
    switch (QMIType) {
        ...
        case QMUX_TYPE_UIM_HTTP : dbg_time("Get clientUIMHTTP = %d", ClientId); break;
...
static void * GobiNetThread(void *pData) {
    ...
    dbg_time("Try to get uim http qmi client id");
    qmiclientId[QMUX_TYPE_UIM_HTTP] = GobiNetGetClientID(qcqmi, QMUX_TYPE_UIM_HTTP);

    qmidevice_send_event_to_manager(RIL_INDICATE_DEVICE_CONNECTED);
...
__GobiNetThread_quit:
    GobiNetDeInit();
    qmidevice_send_event_to_manager(RIL_INDICATE_DEVICE_DISCONNECTED);
    QmiThreadRecvQMI(NULL); //main thread may pending on QmiThreadSendQMI()
